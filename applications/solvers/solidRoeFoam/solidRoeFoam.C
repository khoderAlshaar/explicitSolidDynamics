...
int main(int argc, char *argv[])
{
    ...
    const volVectorField diffLm = lm - lm.oldTime();
    const volTensorField diffP = P - P.oldTime();

    // Number of cells
    const scalar N = diffLm.internalField().size();

    const scalar L2norm_lm =
    Foam::sqrt
    (
        Foam::gSum
        (
            ...
            (
                Foam::magSqr(diffLm.internalField())
            )

        )/N
    );
    const scalar L2norm_P =
    Foam::sqrt
    (
        Foam::gSum
        (
            DimensionedField<scalar, volMesh>
            (
                Foam::magSqr(diffP.internalField())
            )
    ...
    Info<< "linear momentum L2 Residual: "
        << Foam::log10(max(L2norm_lm, SMALL)) << endl
        << "stress L2 Residual: "
        << Foam::log10(max(L2norm_P, SMALL)) << endl
        << endl;
